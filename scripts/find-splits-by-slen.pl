#!/usr/bin/perl

use 5.010;
use strict;
use warnings;
use utf8;
use open qw(:std :utf8);
use JSON::XS qw(decode_json);
use Encode qw(encode_utf8);

my $MAXLSIL = 1;    # set split point max this many seconds prior to word end

my $file_end;
my @sils;

SUB:
while (<>) {
    chomp;
    my ($start, $end, $slen, $predword, $goldword, $goldlineno, $score) = split /\t/;
    $file_end = $end;
    next SUB if $slen <= 0;

    my $pad = $slen / 2;
    $pad = $MAXLSIL if $pad > $MAXLSIL;

    my $mid = $end - $pad;
    push @sils, {
        len => $slen,
        mid => $mid,
    };
}

my $maxi = $#sils;
for my $i (1 .. $maxi) {
    $sils[$i]{l} = $sils[$i - 1];
}
for my $i (0 .. $maxi - 1) {
    $sils[$i]{r} = $sils[$i + 1];
}
$sils[ 0]{l} = { mid => 0 };
$sils[-1]{r} = { mid => $file_end };

my @to_filter = sort {$a->{len} <=> $b->{len}} @sils;

my @filtered;
for my $sil (@to_filter) {
    if ($sil->{r}{mid} - $sil->{l}{mid} > 30) {
        push @filtered, $sil;
    }
    else {
        $sil->{l}{r} = $sil->{r};
        $sil->{r}{l} = $sil->{l};
    }
}

@to_filter = @filtered;
@filtered = ();
for my $sil (@to_filter) {
    if ($sil->{mid} - $sil->{l}{mid} < 12 or
        $sil->{r}{mid} - $sil->{mid} < 12
    ) {
        $sil->{l}{r} = $sil->{r};
        $sil->{r}{l} = $sil->{l};
    }
    else {
        push @filtered, $sil;
    }
}

for my $point (sort {$a <=> $b} map $_->{mid}, @filtered) {
    printf "%.2f\n", $point + 0.0001;
}
